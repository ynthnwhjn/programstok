/******/ (() => { // webpackBootstrap
/*!*********************************************************************!*\
  !*** ./resources/views/purchase_invoices/purchase_invoices_form.js ***!
  \*********************************************************************/
(function () {
  'use strict';

  angular.module('app').controller('PurchaseInvoiceFormController', PurchaseInvoiceFormController);
  function PurchaseInvoiceFormController($scope, $http, toastr) {
    ...
    $scope.browseSupplier = function () {
      return {
        ...
        onSelect: function onSelect($item, $model, $label, $event) {
          console.log($item);
          $scope.formfield.supplier = $item;
          $scope.formfield.supplier_id = $item.id;
        }
      };
    };
    ...
    if ($scope.formfield.purchase_invoice_detail) {
      $scope.gridform1.data = $scope.formfield.purchase_invoice_detail;
    }
    $scope.save = function (formCtrl, evt) {
      evt.preventDefault();
      console.log($scope.formfield);
      console.log($scope.gridform1.data);
      $scope.formfield.purchase_invoice_detail = $scope.gridform1.data;
